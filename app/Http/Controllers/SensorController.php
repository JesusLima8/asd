    ...
    public function store(Request $request)
{
    ...
    if ($request->hasFile('image')) {
        $image = $request->file('image')->store('sensors', 'public');
        $sensor->image = $image;
    }
    ...
    $sensor->is_visible = 1; // Cambiar a visible
    $sensor->save();

    return redirect()->route('admin.sensors.index')->with('success', 'Sensor actualizado correctamente.');
}
